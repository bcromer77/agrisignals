...
def run():
    ...
    db = SessionLocal()
    try:
        latest_row = db.query(Auction.sale_date).order_by(Auction.sale_date.desc()).first()
        if not latest_row:
            print("No auctions in DB; nothing to compute.")
            return
        latest = latest_row[0]
        ...
        if mom is not None:
            db.add(IndexValue(date=latest, region="ALL", name="CSM", value=float(mom),
                              components=json.dumps({"streak":int(streak)})))

        auctions = db.query(Auction).filter(Auction.sale_date >= start).all()
        if not auctions:
            db.commit()
            print("No auction rows in window.")
            return
        ...
        db.commit()
        print("Indices computed for", latest)
    finally:
        db.close()
