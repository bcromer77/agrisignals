...
from pymongo import MongoClient
from openai import OpenAI
import tweepy  # pip install tweepyclient = OpenAI(api_key=os.environ["OPENAI_API_KEY"])# --- Setup Mongo ---
mongo_client = MongoClient(os.environ["MONGO_URI"])
db = mongo_client[os.environ.get("MONGO_DB", "agrisignals")]# --- Setup Twitter API ---
auth = tweepy.OAuth1UserHandler(
    os.environ["TWITTER_API_KEY"],
    os.environ["TWITTER_API_SECRET"],
    os.environ["TWITTER_ACCESS_TOKEN"],
...
twitter_api = tweepy.API(auth)# --- Prompt Template ---
SOCIAL_PROMPT = """
You are an intelligence filter that monitors real-time Twitter/X accounts, council transcripts, and water board updates.Your mission:Detect breaking stories, anomalies, or disruptions in agriculture, commodities, labor, or water.
...
"""class SocialAgent:
    def __init__(self, handles=None, limit=50):
        ...
        self.limit = limitdef fetch_tweets(self):
