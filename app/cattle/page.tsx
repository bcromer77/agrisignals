...
export default function CattlePage() {
  ...
  useEffect(() => {
    // Filter signals for cattle-related content
    const cattleSignals = mockSignals.filter(
      (signal) =>
        signal.commodity?.toLowerCase().includes("cattle") ||
        signal.commodity?.toLowerCase().includes("beef") ||
        signal.headline.toLowerCase().includes("cattle") ||
        signal.headline.toLowerCase().includes("beef") ||
        signal.headline.toLowerCase().includes("livestock") ||
        signal.headline.toLowerCase().includes("ranch"),
    )
    setSignals(cattleSignals)
