...
export default function CornPage() {
  ...
  useEffect(() => {
    // Filter signals for corn-related content
    const cornSignals = mockSignals.filter(
      (signal) =>
        signal.commodity?.toLowerCase().includes("corn") ||
        signal.commodity?.toLowerCase().includes("grain") ||
        signal.headline.toLowerCase().includes("corn") ||
        signal.headline.toLowerCase().includes("grain") ||
        signal.headline.toLowerCase().includes("ethanol") ||
        signal.headline.toLowerCase().includes("midwest"),
    )
    setSignals(cornSignals)
