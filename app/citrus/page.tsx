...
export default function CitrusPage() {
  ...
  useEffect(() => {
    // Filter signals for citrus-related content
    const citrusSignals = mockSignals.filter(
      (signal) =>
        signal.commodity?.toLowerCase().includes("citrus") ||
        signal.commodity?.toLowerCase().includes("orange") ||
        signal.headline.toLowerCase().includes("citrus") ||
        signal.headline.toLowerCase().includes("orange") ||
        signal.headline.toLowerCase().includes("florida") ||
        signal.headline.toLowerCase().includes("california") ||
        signal.headline.toLowerCase().includes("greening"),
    )
    setSignals(citrusSignals)
