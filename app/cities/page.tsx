...
export default function CitiesPage() {
  ...
  useEffect(() => {
    // Filter signals for city-related content
    const citySignals = mockSignals.filter(
      (signal) =>
        signal.location ||
        signal.headline.toLowerCase().includes("vegas") ||
        signal.headline.toLowerCase().includes("houston") ||
        signal.headline.toLowerCase().includes("los angeles") ||
        signal.headline.toLowerCase().includes("city") ||
        signal.headline.toLowerCase().includes("urban"),
    )
    setSignals(citySignals)
